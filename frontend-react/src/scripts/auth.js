...
<div id="nbOv" class="nb-ov" hidden role="dialog" aria-modal="true" aria-labelledby="nbTitle">
  ...
  function openSettings(tab = 'account') {
    const ov = $('accountModalOverlay'); if (!ov) return; ov.classList.add('open');
  ...
  function closeSettings() { const ov = $('accountModalOverlay'); if (ov) ov.classList.remove('open'); }
  function openSignout()   { const el = $('signoutConfirm'); if (el) el.classList.add('open'); }
  function closeSignout()  { const el = $('signoutConfirm'); if (el) el.classList.remove('open'); }
...
`;
  ...
  /* ════════════════════════════════════════════════════
     setupDOM — call after React renders so that elements
     like #profileIconBtn, #accountModalClose etc. exist.
     Safe to call multiple times (uses removeEventListener
     pattern via cloneNode replacement isn't needed because
     these selectors only exist once in the DOM).
  ════════════════════════════════════════════════════ */
  function setupDOM() {
    setupDropdown();
    $('dropdownSignoutBtn')?.addEventListener('click', () => { $('profileDropdown')?.classList.remove('open'); openSignout(); });
  ...
  window.Auth = { open: openModal, close: closeModal, panel: goPanel, isAuthenticated: () => !!S.access, getUser: () => S.user, authFetch, apiFetch: authFetch, logout, ready: authReady, setupDOM };
